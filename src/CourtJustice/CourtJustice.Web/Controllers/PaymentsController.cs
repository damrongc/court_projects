                ...
                if (files[0]?.Length != 0)
                {
                    ...
                    for (int i = 0; i < compDt.Rows.Count; i++)
                    {
                        ...
                        var payment = new PaymentExcelViewModel
                        {
                            ...
                            ReceiptDate = dt.Rows[i][5].ToString().ConvertDateFormatTHToUS(),
                            BookingDate = dt.Rows[i][6].ToString().ConvertDateFormatTHToUS(),
                            TotalReceived = dt.Rows[i][7].ToString().ToDecimal(),
                            WOBalance = dt.Rows[i][8].ToString().ToDecimal(),
                            StartOverdueStatus = dt.Rows[i][9].ToString().Trim(),
                            EndOverdueStatus = dt.Rows[i][10].ToString().Trim(),
